...
import { groupIncomeByDay, sumIncomeInRange } from "@/features/incomes/data/incomes.repo";
...
function emptyDailyFlow(d: Date): DailyFlow[] {
  ...
  return Array.from({ length: end.getDate() }, (_, i) => ({ day: i + 1, income: 0, expense: 0 }));
}
...
export async function getDashboardData(
  ...
  const balance = Number(allTimeIncome._sum.amount ?? 0) - Number(paidDebtsAgg._sum.amount ?? 0);
