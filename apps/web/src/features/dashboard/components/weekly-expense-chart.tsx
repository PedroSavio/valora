...
function buildWeeklyExpenses(data: DailyFlow[]): WeekExpense[] {
  ...
  return Array.from({ length: totalWeeks }, (_, weekIndex) => {
    ...
    const value = data.slice(start, start + 7).reduce((sum, day) => sum + day.expense, 0);
    return {
  ...
  return (
    <section className="h-full rounded-[22px] border border-border bg-card p-6">
      <header className="mb-6">
        <h2 className="text-lg font-semibold">Distribuição semanal</h2>
          ...
          return (
            <div key={week.label} className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-semibold text-foreground">{formatBRL(week.value)}</span>
              </div>
