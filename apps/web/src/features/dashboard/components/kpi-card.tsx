...
export function KpiCard({ label, value, delta, tone = "neutral", hint }: Props) {
  const mutedClass = tone === "primary" ? "text-primary-foreground/70" : "text-muted-foreground";
  return (
    <div className={`rounded-[22px] border p-7 lg:p-8 ${toneStyles[tone]}`}>
      ...
      <div className="mt-5 overflow-x-auto whitespace-nowrap text-[clamp(1.35rem,3vw,2.25rem)] leading-tight font-semibold tracking-tight [scrollbar-width:none]">
        {formatBRL(value)}
      </div>
...
function DeltaBadge({ delta, tone }: { delta: KpiDelta; tone: Tone }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs ${bg}`}>
